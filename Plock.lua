...
spawn(function()
	for v708, v709 in pairs(game:GetDescendants()) do
		if (v709:IsA("ParticleEmitter") or v709:IsA("Trail")) then
			v709.Lifetime = NumberRange.new(0, 0)
...
v3.Window = function(v209, v210, v211, v212)
	...
	v346.T = function(v718, v719, v720)
		...
		v772.Toggle = function(v1135, v1136, v1137, v1138, vDescription)
	local v1139 = false
	...
	v1140.Size = UDim2.new(0, 38, 0, vDescription and 48 or 30)
	...
	v1141.Size = UDim2.new(0, 308, 0, vDescription and 48 or 30)
	...
	if vDescription then
		...
		Desc.Position = UDim2.new(0, 50, 0, 22)
		Desc.Size = UDim2.new(0, 240, 0, 20)
		Desc.Font = Enum.Font.Gotham
		Desc.Text = vDescription
		Desc.TextSize = 10
	...
	local function SetState(state)
		v1139 = state
		v1148.ImageRectOffset = state and Vector2.new(4, 836) or Vector2.new(940, 784)
		game.TweenService:Create(v1148, TweenInfo.new(0.08), {
			ImageColor3 = state and Color3.fromRGB(255,225,225) or Color3.fromRGB(128,102,255)
		}):Play()
		pcall(v1138, v1139)
	end
	...
	if v1137 then
		...
		v772.Textbox = function(v1588, v1589, v1590, v1591, v1592desc)
			...
			v1592.Size = UDim2.new(0, 310, 0, v1592desc and 45 or 31);
...
v53:Button("Copy Discord Link", function()
	setclipboard("https://discord.gg/C9UwfeEkm8");
end);
v53:Seperator("Time");
...
-- AUTO DUNGEON COMPLETO (FLY + ALLY FILTER + AUTO RESTART + RESPAWN FIX)
...
-- CONFIGURAÇÕES
local FLY_LERP_SPEED = 0.15     -- velocidade segura (0.15 ~ 0.22)
local HEIGHT_ABOVE_NPC = 25      -- altura acima do inimigo

v54:Seperator("Dungeon Event")
...
v54:Toggle(
	...
	function(v)
		...
		if _G.AutoDungeon and game.PlaceId ~= DungeonPlaceId then
			_G.AutoDungeon = false
			StarterGui:SetCore("SendNotification",{
				Title = "Aviso",
				Text = "Entre na dungeon primeiro",
				Duration = 5
			})
		end
	end,
	"Mata automaticamente os inimigos da Dungeon. Não funciona fora da Dungeon."
)
...
-- FILTRO DE INIMIGO REAL (IGNORA ALLY / LOVE / FRIENDLY)
local function IsValidEnemy(mob)
	...
	if mob:FindFirstChild("Ally") then return false end
	if mob:FindFirstChild("Friendly") then return false end
	if mob:FindFirstChild("Team") then return false end
	if mob:FindFirstChild("Owner") then return false end

	return true
end

-- DETECTA MORTE E REINICIA O AUTO DUNGEON
...
task.spawn(function()
	while task.wait(0.1) do
		...
		if not IsAlive() then
			ResetFarm()
			repeat task.wait() until IsAlive() or not _G.AutoDungeon
		end
		...
		for _, mob in pairs(Workspace.Enemies:GetChildren()) do
			...
			if not IsValidEnemy(mob) then continue end
			...
			repeat
				...
				if not _G.AutoDungeon then break end
				if not IsValidEnemy(mob) then break end
				...
				local hrp = GetHRP()
				if hrp then
					local targetCF = mob.HumanoidRootPart.CFrame * CFrame.new(0, HEIGHT_ABOVE_NPC, 0)
					hrp.CFrame = hrp.CFrame:Lerp(targetCF, FLY_LERP_SPEED)
					hrp.Velocity = Vector3.zero
				end

				mob.HumanoidRootPart.CanCollide = false
				mob.HumanoidRootPart.Size = Vector3.new(45,45,45)
		...
		-- RESTART AUTOMÁTICO SE TRAVAR (ILHAS / BOSS / MECÂNICAS)
		if _G.AutoDungeon and not foundEnemy then
			ResetFarm()
			task.wait(0.4)
		end
	end
...
spawn(function()
    ...
    while wait(0.5) do
        if player and player:FindFirstChild("PlayerGui") then
            local gui = player.PlayerGui:FindFirstChild("DamageCounter", true)
            if gui and gui:IsA("ScreenGui") then
                gui.Enabled = _G.KobenHeegeen == false
            end
        end
    end
end)
...
spawn(function()
    while wait(0.5) do
        if _G.WhiteScreen then
            local world = game.Workspace:FindFirstChild("_WorldOrigin")
            if world then
                for _, obj in pairs(world:GetChildren()) do
                    if obj.Name == "CurvedRing" or obj.Name == "SlashHit" or obj.Name == "DamageCounter" or obj.Name == "SwordSlash" or obj.Name == "SlashTail" or obj.Name == "Sounds" then
                        obj:Destroy()
                    end
                end
            end
...
function FastAttack:GetBladeHits()
	...
	local function ProcessFolder(folder)
		for _, v in pairs(folder:GetChildren()) do
			if v ~= char and self:IsEntityAlive(v) then
				local part = v:FindFirstChild(Config.HitboxLimbs[math.random(#Config.HitboxLimbs)]) or v:FindFirstChild("HumanoidRootPart")
...
function FastAttack:ShootInTarget(targetPos)
	...
	local shootType = self.SpecialShoots[tool.Name] or "Normal"
	if shootType == "Position" or (shootType == "TAP" and tool:FindFirstChild("RemoteEvent")) then
		...
		GunValidator:FireServer(self:GetValidator())
...
function FastAttack:Attack()
	...
	if not table.find({"Melee","Blox Fruit","Sword","Gun"}, tool.ToolTip) then return end
...
RunService.Stepped:Connect(function()
	Config.AutoClickEnabled = AutoAttackEnabled
	Config.AttackMobs = AttackMobs
	Config.AttackPlayers = AttackPlayers
...
-- ===== TOGGLES =====
v55:Toggle("AutoAttack", true, function(v)
	AutoAttackEnabled = v
end)

v55:Toggle("Attack Mobs", true, function(v)
	AttackMobs = v
end)

v55:Toggle("Attack Players", true, function(v)
	AttackPlayers = v
end)
...
v59:Toggle(" Enable Aimbot Skill", false, function(v)
	AimbotEnabled = v
...
v59:Toggle("Enable Aimbot on Player", false, function(v)
	AimPlayers = v
...
v59:Toggle("Enabled Aimbot on NPCs", false, function(v)
	AimMobs = v
...
--// WALK SPEED & JUMP POWER (v59 compatível)
local RunService = game:GetService("RunService")
local Players = game:GetService("Players")
local player = Players.LocalPlayer
...
local function applySpeed()
    if _G.WSEnabled and humanoid then
        pcall(function() humanoid.WalkSpeed = _G.WalkSpeedValue end)
    end
end

local function applyJump()
    if _G.JPEnabled and humanoid then
        pcall(function() humanoid.JumpPower = _G.JumpPowerValue end)
    end
end
...
local function bindHumanoid(h)
    ...
    if humanoid then
        propertyConnection = humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if _G.WSEnabled and humanoid.WalkSpeed ~= _G.WalkSpeedValue then
...
RunService.Heartbeat:Connect(function()
    if humanoid then
        ...
        if _G.JPEnabled and humanoid.JumpPower ~= _G.JumpPowerValue then
            humanoid.JumpPower = _G.JumpPowerValue
        end
    end
end)

--// v59 TOGGLES E SLIDERS
v59:Toggle("WalkSpeed Enabled", false, function(v)
    _G.WSEnabled = v
    applySpeed()
end)

v59:Slider1("WalkSpeed", 1, 500, 48, function(value)
    _G.WalkSpeedValue = value
    applySpeed()
end)

v59:Toggle("JumpPower Enabled", false, function(v)
    _G.JPEnabled = v
    applyJump()
end)

v59:Slider1("JumpPower", 10, 500, 50, function(value)
...
v61:Toggle("Auto Select Raids", _G.SelectDungeon, function(v659)
